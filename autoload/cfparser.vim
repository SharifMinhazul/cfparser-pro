...
function! cfparser#CFDownloadTests() "{{{
    ...
    if empty(match)        
		...
		let cnt = 0
        for test in tests
			silent call cfparser#CFLog(test[0], printf(expand('%:p:h') . "/%s%d.in", problem, cnt))
			silent call cfparser#CFLog(test[1], printf(expand('%:p:h') . "/%s%d.out", problem, cnt))
            let cnt += 1
		endfor	
        echon "\r\r"
...
function! cfparser#CFLogin() "{{{
    let l:cf_uname = input('username: ')
    let l:cf_passwd = inputsecret('password: ')
    ...
    let cf_response = system(printf("curl --location --silent --cookie-jar %s --cookie %s --data 'action=enter&handle=%s&remember=%s&csrf_token=%s' --data-urlencode 'password=%s' '%s://%s/enter'", g:cf_cookies_file, g:cf_cookies_file, l:cf_uname, l:cf_remember, csrf_token, l:cf_passwd, s:cf_proto, s:cf_host))
    echon "\r\r"
...
function! cfparser#CFRun() "{{{
    ...
    execute "!clear && g++ -Wall -std=c++14 %:p -o /tmp/cfparser_exec && /tmp/cfparser_exec"
